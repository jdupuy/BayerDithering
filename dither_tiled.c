...
void Usage(const char *appName)
{
    ...
    LOG("usage: %s bayerMatrixSize colorsPerChannel pathToFile", appName);
}
...
int main(int argc, const char **argv)
{
    ...
    bayerMatrixSize = atoi(argv[1]);
    colorsPerChannel = atoi(argv[2]);
    pathToFile = argv[3];
    ...
    bayerMatrixSizeLg = FindMSB(bayerMatrixSize);
    ...
    for (int32_t y = 0; y < input.height; ++y) {
        for (int32_t x = 0; x < input.width; ++x) {
            for (int32_t c = 0; c < input.components; ++c) {
                ...
                const float v = BayerMatrixCoefficient(x % bayerMatrixSize,
                                                       y % bayerMatrixSize,
                                                       bayerMatrixSizeLg)
                              / (float)(bayerMatrixSize * bayerMatrixSize);
